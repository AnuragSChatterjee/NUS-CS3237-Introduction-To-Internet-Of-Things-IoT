...
void loop() {
  ...
  VRx = voltageread(ADS1115_COMP_0_GND);

  // 1 is the Y-direction
  VRy = voltageread(ADS1115_COMP_1_GND);

  BUTTONSTATE = digitalRead(PUSHBUTTON);

  outputs(VRx,VRy,BUTTONSTATE); //final output is here 
}
//create a function to read the X and Y voltages 
float voltageread(ADS1115_MUX channel) {
  float voltage = 0.00;
  adc.setCompareChannels(channel);
  v_out = adc.getResult_V(); 
  return v_out;
}

void outputs(int X, int Y, int button) {
  float VRx_at_rest = 1.65;
  float VRy_at_rest = 1.66;

  if (VRx_at_rest == X && VRy_at_rest == Y) {
    Serial.println("Joystick is at rest");
    
  } else if (X < VRx_at_rest && Y == VRy_at_rest) {
    Serial.println("Joystick is at the LEFT position"); //VRx decreases upon being pushed to the left
    
  } else if (X > VRx_at_rest && Y == VRy_at_rest) {
    Serial.println("Joystick is at the RIGHT position"); //VRx increases upon being pushed to the right
    
  } else if (X == VRx_at_rest && Y > VRy_at_rest) {
    Serial.println("Joystick is at the DOWN position"); //VRy increases upon being pushed down
    
  } else if (X == VRx_at_rest && Y < VRy_at_rest) {
    Serial.println("Joystick is at the UP position"); //VRy decreases upon being pushed up
    
  } else if (X > VRx_at_rest && Y < VRy_at_rest) {
    Serial.println("Joystick is at the UP-RIGHT position");
    
  } else if (X < VRx_at_rest && Y < VRy_at_rest) {
    Serial.println("Joystick is at the UP-LEFT position");
    
  } else if (X > VRx_at_rest && Y > VRy_at_rest) {
    Serial.println("Joystick is at the DOWN-RIGHT position");
    
  } else if (X < VRx_at_rest && Y > VRy_at_rest) {
    Serial.println("Joystick is at the DOWN-LEFT position");
